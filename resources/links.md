---
artifact_type: link_collection
date: 2025-12-06
tags:
  - space/kinen
  - type/reference
summary: "Curated web links for kinen development"
---

# Curated Links

## Vector Databases

### LanceDB
- **Homepage**: https://lancedb.github.io/lancedb/
- **GitHub**: https://github.com/lancedb/lancedb
- **Full-Text Search**: https://lancedb.github.io/lancedb/fts/
- **Hybrid Search**: https://lancedb.github.io/lancedb/hybrid_search/
- **Node.js Client**: https://lancedb.github.io/lancedb/js/

### Alternatives (for reference)
- **ChromaDB**: https://docs.trychroma.com/
- **Qdrant**: https://qdrant.tech/documentation/
- **sqlite-vec**: https://github.com/asg017/sqlite-vec

## Embeddings

### Models
- **Sentence Transformers**: https://www.sbert.net/
- **all-MiniLM-L6-v2**: https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2
- **OpenAI Embeddings**: https://platform.openai.com/docs/guides/embeddings

### Local Inference
- **Ollama**: https://ollama.ai/
- **@xenova/transformers**: https://github.com/xenova/transformers.js

## Obsidian

### Core
- **Obsidian**: https://obsidian.md/
- **Plugin API**: https://docs.obsidian.md/Plugins/Getting+started/Build+a+plugin
- **URI Protocol**: https://help.obsidian.md/Advanced+topics/Using+Obsidian+URI

### Community Plugins
- **Dataview**: https://blacksmithgu.github.io/obsidian-dataview/
- **Templater**: https://silentvoid13.github.io/Templater/
- **QuickAdd**: https://github.com/chhoumann/quickadd

## Knowledge Management

### Methodology
- **Zettelkasten**: https://zettelkasten.de/overview/
- **Building a Second Brain**: https://www.buildingasecondbrain.com/
- **Evergreen Notes**: https://notes.andymatuschak.org/Evergreen_notes

### Research
- **Free Energy Principle**: https://en.wikipedia.org/wiki/Free_energy_principle
- **Memory Consolidation**: https://en.wikipedia.org/wiki/Memory_consolidation
- **Active Inference**: https://en.wikipedia.org/wiki/Free_energy_principle#Active_inference

## Tools Referenced

### Semantic Search
- **ck (BeaconBay)**: https://github.com/BeaconBay/ck — Semantic code search
- **ripgrep**: https://github.com/BurntSushi/ripgrep — Fast text search

### Issue Tracking
- **beads**: https://github.com/steveyegge/beads — Git-backed issue tracker

## Papers to Find

- Friston, K. (2010). "The free-energy principle: a unified brain theory?"
- Walker, M. (2017). "Why We Sleep" — Memory consolidation during sleep
- Luhmann, N. — Original Zettelkasten papers

## See Also

- [[index]] — Full resources index
- [[notes/]] — Web clips and notes


