...
1. Get schema (fields, associations)
2. Get record count
3. Infer semantic type from patterns
4. Check if documented

For each custom event:
1. Get schema
2. Get volume (30d)
3. Check contact linkage
4. Infer semantic type

## Round 2+: Data Landscape Questions
- Confirm/correct object semantics
- Confirm/correct event semantics
- Document undocumented objects
- Identify key fields

## Output
- Update: artifacts/discovery/data-inventory.yaml
```
...
- [ ] Multiple rounds work (if needed for complex workspace)
...
```markdown
# domains/lifecycle.md

## Concepts

### Active Customers
- Definition: Company with active subscription
- Standard predicate: subscription.status = 'active'
- Data requirements: subscription object with status field

### Churned Customers
- Definition: Company that cancelled
- Standard predicate: subscription.status = 'cancelled'
- Ideal: Also has churnedAt timestamp
...
```
...
## Per Domain
For each domain (lifecycle, engagement, intent, value):
1. Check what data supports it
2. Propose definitions with thresholds
3. Ask user to validate/adjust

## Round Format
- Present: "Based on your data, here's how I'd define [concept]"
- Ask: "Is this threshold right for your business?"
- Options: Adjust threshold, use different field, skip concept

## Output
- Create: artifacts/mapping/semantic-mapping.yaml
```
...
## Gap Detection
For each concept in semantic mapping:
- If status = "blocked": Add to gap list
- Identify: What field/object is missing
- Identify: Where could that data come from

## Round Format
- Present: Gap summary table
- For each critical gap:
  - Explain what's blocked
  - Propose resolution options
  - Ask: Priority? Workaround acceptable?

## Output
- Create: artifacts/analysis/gap-analysis.md
...
- [ ] Gap analysis artifact useful for next steps
...
└── datahub/
    └── views/
        └── active-customers-view.json
```

#### Agent Additions

```markdown
# Phase 5: Segment Catalog

## Round Format
- Present: All segments ready to create
- Show: Name, predicate, folder
- Ask: Confirm before creation
- Ask: Any naming adjustments?

## Output
- Create: artifacts/catalog/segment-catalog.yaml

# Phase 6: Implementation

## Actions
1. Create audiences via Bird API
2. Create DataHub views
3. Generate session summary

## Output
- Create: bundle/ with all deployable files
...
Based on session artifacts:
1. Overview article (what segments exist)
2. Composition guide (how to combine)
3. Per-domain reference (detailed definitions)
4. Data model documentation (underlying data)

## Output
- Add to bundle: knowledge-base/articles/
- Publish to workspace via API
```
...
This workspace has a **SaaS Semantic Layer** — a set of pre-built audience 
...
- **Win-back campaign** → Use "Churned Customers" segment
- **Engagement nurture** → Use "Dormant Users" segment
...
All segments are in: **Audiences → SaaS Semantic Layer/**
...
The SaaS Semantic Layer provides **building blocks** — simple segments 
...
= Churned customers who didn't visit cancel page
  (possible involuntary churn — payment failed?)
```
...
1. **Start simple** — One segment often works
2. **Add constraints gradually** — Don't over-filter
3. **Check sizes** — Ensure audience is large enough
4. **Test compositions** — Preview before sending
...
| `startDate` | timestamp | — | When subscription began |
...
```
crm.contact ──────► crm.company (via companyRefs)
     │
     └──────► custom-event.user-login (via userId)
     │
     └──────► custom-event.feature-click (via userId)

custom-object.subscription ──────► crm.company (via companyId)
```
...
---
*Part of SaaS Semantic Layer — Updated 2025-12-11*
```
...
│   ▼       ▼       ▼           ▼           ▼           ▼                   │
│ ┌──────┐┌──────┐┌──────┐ ┌──────────┐ ┌──────────┐ ┌──────┐              │
│ │AUDIE-││DATA- ││ASSOC-│ │KNOWLEDGE │ │ COMPUTED │ │MANUAL│              │
│ │NCES  ││HUB   ││IATIO-│ │   BASE   │ │  FIELDS  │ │ACTION│              │
│ │(Auto)││VIEWS ││NS    │ │ ARTICLES │ │ (Review) │ │GUIDE │              │
│ │      ││(Auto)││(Rev) │ │  (Auto)  │ │          │ │      │              │
│ └──┬───┘└──┬───┘└──┬───┘ └────┬─────┘ └────┬─────┘ └──┬───┘              │
│    │       │       │          │            │          │                   │
│    ▼       ▼       ▼          ▼            ▼          ▼                   │
...
These articles ensure the semantic layer is **self-documenting** — anyone in the workspace can understand and use the segments without asking the person who created them.
